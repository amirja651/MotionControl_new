...
PositionController::PositionController(uint8_t motorId, TMC5160Manager& driver,
                                       DirMultiplexer& dirMultiplexer,
                                       uint16_t stepPin, uint16_t enPin,
                                       mae3::Mae3Encoder& encoderMae3)
    : _driver(driver),
      _stepper(AccelStepper::DRIVER, stepPin,
  ...
  _speedProfiles[static_cast<int>(MovementType::SHORT_RANGE)] = {
      2000.0f, 4000.0f};  // Increased from 1000
  _speedProfiles[static_cast<int>(MovementType::MEDIUM_RANGE)] = {
      4000.0f, 8000.0f};  // Increased from 2000
  _speedProfiles[static_cast<int>(MovementType::LONG_RANGE)] = {
      8000.0f, 16000.0f};  // Increased from 4000

  // Initialize distance-based speed profiles for precise control
  // NEGLIGIBLE (< 6 steps) - Not used, but defined for completeness
  _distanceSpeedProfiles[static_cast<int>(DistanceType::NEGLIGIBLE)] = {
      0.0f, 0.0f, 0};

  // VERY_SHORT (6 - 32 steps) - Very slow and precise
  _distanceSpeedProfiles[static_cast<int>(DistanceType::VERY_SHORT)] = {
      500.0f, 1000.0f, 6};

  // SHORT (32 - 64 steps) - Slow and precise
  _distanceSpeedProfiles[static_cast<int>(DistanceType::SHORT)] = {1000.0f,
                                                                   2000.0f, 32};

  // MEDIUM (64 - 320 steps) - Balanced speed and precision
  _distanceSpeedProfiles[static_cast<int>(DistanceType::MEDIUM)] = {
      2000.0f, 4000.0f, 64};

  // LONG (320 - 640 steps) - Moderate speed
  _distanceSpeedProfiles[static_cast<int>(DistanceType::LONG)] = {4000.0f,
                                                                  8000.0f, 320};

  // VERY_LONG (> 640 steps) - Higher speed for longer distances
  _distanceSpeedProfiles[static_cast<int>(DistanceType::VERY_LONG)] = {
      8000.0f, 16000.0f, 640};
}
...
bool PositionController::begin() {
  ...
  if (!_dirMultiplexer.begin()) {
    log_e("Failed to initialize direction multiplexer for motor %d",
          _motorId + 1);
    return false;
  }
  ...
  _stepper.setMaxSpeed(
      _speedProfiles[static_cast<int>(MovementType::MEDIUM_RANGE)].maxSpeed);
  _stepper.setAcceleration(
      _speedProfiles[static_cast<int>(MovementType::MEDIUM_RANGE)]
          .acceleration);
...
bool PositionController::moveToSteps(int32_t targetSteps,
  ...
  if (!isValidMovementDistanceSteps(movementDistanceSteps)) {
    log_w(
        "Motor %d: Movement distance %d steps is negligible (< 6 steps), "
        "ignoring",
        _motorId + 1, movementDistanceSteps);
    return false;
  }
...
bool PositionController::moveRelativeSteps(int32_t deltaSteps,
                                           MovementType movementType,
                                           ControlMode controlMode) {
  if (!_enabled || !_initialized) return false;
...
void PositionController::stop() {
  ...
  _stepper.setCurrentPosition(
      _stepper.currentPosition());  // Update current position
...
bool PositionController::isAtTarget() const {
  ...
  return abs(static_cast<int32_t>(currentPos - targetPos)) <=
         POSITION_ACCURACY_STEPS;
}
...
int32_t PositionController::getCurrentSteps() const {
  ...
  return static_cast<int32_t>(
      const_cast<AccelStepper&>(_stepper).currentPosition());
}
...
void PositionController::setSpeedProfile(MovementType type, float maxSpeed,
                                         float acceleration) {
...
void PositionController::startPositionControlTask() {
  if (_positionControlTaskHandle != nullptr) return;
...
bool PositionController::queueMovementCommand(const MovementCommand& command) {
  if (_movementCommandQueue == nullptr) return false;

  return xQueueSend(_movementCommandQueue, &command, pdMS_TO_TICKS(100)) ==
         pdTRUE;
}
...
bool PositionController::executeMovement(const MovementCommand& command) {
  ...
  const char* modeStr =
      (command.controlMode == ControlMode::OPEN_LOOP) ? "OPEN-LOOP" : "HYBRID";
  ...
  log_d("Motor %d moving to %d steps (distance: %d steps, type: %s, mode: %s)",
        _motorId + 1, targetSteps, command.movementDistanceSteps, distanceStr,
        modeStr);

  return true;
}

float PositionController::calculateOptimalSpeedSteps(int32_t distanceSteps,
                                                     MovementType type) {
...
void PositionController::runPositionControl() {
  ...
  if (_status.isMoving && _stepper.distanceToGo() == 0) {
    ...
    const char* modeStr =
        (_controlMode == ControlMode::OPEN_LOOP) ? "OPEN-LOOP" : "HYBRID";
...
ControlMode PositionController::getControlMode() const { return _controlMode; }
  ...
  if (_encoderMae3 != nullptr) {
    if (_encoderMae3->enable() == mae3::Status::Ok) {
      ...
      if (_encoderMae3->tryGetPosition(pos, ton, toff)) {
        Serial.println(pos);
        return {
            ton, toff, 0, static_cast<uint32_t>(pos), 0, Direction::UNKNOWN};
      }
    }
  }
  Serial.println("000");
...
void PositionController::applyHybridModeCorrection() {
  if (!isHybridModeEnabled() || !_status.isMoving) return;
  ...
  switch (mode) {
    ...
    case ControlMode::HYBRID:
      ...
      if (_encoderMae3 != nullptr &&
        ...
        log_d(
            "Motor %d: Hybrid mode - starting position from encoder: %d steps",
            _motorId + 1, encoderSteps);
      } else {
        log_w("Motor %d: Cannot enable hybrid mode - encoder not available",
              _motorId + 1);
        _controlMode = ControlMode::OPEN_LOOP;
        _status.controlMode = ControlMode::OPEN_LOOP;
      }
...
DistanceType PositionController::calculateDistanceTypeSteps(
...
void PositionController::setDistanceBasedSpeedProfile(
    DistanceType distanceType) {
  if (distanceType == DistanceType::NEGLIGIBLE) return;
...
float PositionController::calculateOptimalSpeedForDistanceSteps(
    int32_t distanceSteps) {
  DistanceType distanceType = calculateDistanceTypeSteps(distanceSteps);
  if (distanceType == DistanceType::NEGLIGIBLE) return 0.0f;
  ...
  return _distanceSpeedProfiles[static_cast<int>(DistanceType::MEDIUM)]
      .maxSpeed;
}

float PositionController::calculateOptimalAccelerationForDistanceSteps(
    int32_t distanceSteps) {
  DistanceType distanceType = calculateDistanceTypeSteps(distanceSteps);
  if (distanceType == DistanceType::NEGLIGIBLE) return 0.0f;
...
void PositionController::configureSpeedForDistanceSteps(int32_t distanceSteps) {
  if (!isValidMovementDistanceSteps(distanceSteps)) return;

  float optimalSpeed = calculateOptimalSpeedForDistanceSteps(distanceSteps);
  float optimalAcceleration =
      calculateOptimalAccelerationForDistanceSteps(distanceSteps);

  _stepper.setMaxSpeed(optimalSpeed);
  _stepper.setAcceleration(optimalAcceleration);

  log_d(
      "Motor %d: Distance %d steps - Speed: %.0f steps/s, Accel: %.0f steps/s²",
      _motorId + 1, distanceSteps, optimalSpeed, optimalAcceleration);
}
...
MotorType PositionController::getMotorType() const {
  if (_motorId == 0) return MotorType::LINEAR;
