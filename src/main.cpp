...
enum class VoltageStatus : uint8_t { VOLTAGE_NORMAL, VOLTAGE_DROPPED };
...
VoltageMonitor voltageMonitor(VoltageMonitorPins::POWER_3_3,
                              VoltageMonitor::MonitorMode::DIGITAL_, 0, 10);

// Driver status tracking
static bool driverEnabled[4] = {false, false, false, false};
TMC5160Manager driver[4] = {
    TMC5160Manager(0, DriverPins::CS[0]), TMC5160Manager(1, DriverPins::CS[1]),
...
DirMultiplexer dirMultiplexer(MultiplexerPins::S0, MultiplexerPins::S1,
                              MultiplexerPins::DIR);
...
static constexpr uint8_t MIN_IRUN =
    1;  // Minimum IRUN value (1/32 of RMS current)
static constexpr uint8_t MAX_IRUN =
    31;  // Maximum IRUN value (31/32 of RMS current)

static constexpr uint8_t MIN_IHOLD =
    1;  // Minimum IHOLD value (1/32 of RMS current)
static constexpr uint8_t MAX_IHOLD =
    31;  // Maximum IHOLD value (31/32 of RMS current)
...
inline ControlMode safeModeFromInt(int v,
                                   ControlMode fallback = ControlMode::HYBRID);
bool isMoving();
static void printConversionTable(float inputValue, const ConvertValues& cvfd,
                                 const ConvertValues& cvfp,
                                 const ConvertValues& cvfs,
                                 const ConvertValues& cvfm);
...
void setup() {
  ...
  xTaskCreatePinnedToCore(serialReadTask, "SerialReadTask", 8192, NULL, 2,
                          &serialReadTaskHandle, 0);
...
void storeToMemory() {
  ...
  if (control_save) {
    ...
    log_d(
        ...
        "   • Value   : %d\n"
        "   • Success : %s\n",
        currentIndex + 1, key_cm.c_str(), static_cast<int>(control_mode),
        isSave(w));
  }
  ...
  if (origin_save) {
    ...
    log_d(
        ...
        "   • Value   : %u\n"
        "   • Success : %s\n",
        currentIndex + 1, key_op.c_str(), origin_pulses, isSave(w1),
        key_ot.c_str(), origin_turn, isSave(w2));
  }
  ...
  if (voltageDrop_save) {
    ...
    log_d(
        ...
        "   • Value   : %d\n"
        "   • Success : %s\n",
        currentIndex + 1, key_vdp.c_str(), voltageDrop_pulses, isSave(w1),
        key_vdt.c_str(), voltageDrop_turn, isSave(w2), key_vs.c_str(),
        static_cast<int>(voltage_status), isSave(w3));
  }
  ...
  if (reached_save) {
    ...
    log_d(
        ...
        "   • Value   : %d\n"
        "   • Success : %s\n",
        currentIndex + 1, key_rp.c_str(), reached_pulses, isSave(w1),
        key_rt.c_str(), reached_turns, isSave(w2), key_vs.c_str(),
        static_cast<int>(voltage_status), isSave(w3));
  }
...
uint32_t loadOriginPosition() {
  ...
  uint32_t origin_pulses =
      prefs.getUInt(key_op.c_str(), 0);  // Default to 0 if not found

  log_d(
      "\n⚙️ Motor [%d]\n"
      "   • Origin : %u pulses\n",
      currentIndex + 1, origin_pulses);
...
void loadControlMode() {
  ...
  log_d(
      "\n⚙️ Motor [%d]\n"
      "   • Mode  : %s\n",
      currentIndex + 1, toString(control_mode));
}
...
void loadPosition() {
  ...
  voltage_status = static_cast<VoltageStatus>(
      prefs.getInt(key_voltage_status.c_str(),
                   static_cast<int>(VoltageStatus::VOLTAGE_NORMAL)));
  ...
  log_d(
      ...
      "   • Turn   : %d\n"
      "   • Pulses : %u\n",
      currentIndex + 1, toString(voltage_status), loaded_turns, loaded_pulses);
}
...
void serialReadTask(void* pvParameters) {
  ...
  while (1) {
    while (Serial.available()) {
      ...
      } else if (c == commands[static_cast<int>(CommandKey::Q)]
                              [0])  // 'Q' go to position from memory
      {
        if (!isMoving()) {
          ConvertValues encoderPulses =
              UnitConverter::convertFromPulses(loaded_pulses);
          float target;
          if (isLinearMotor(currentIndex)) {
            target = encoderPulses.TO_MICROMETERS +
                     (loaded_turns * LEAD_SCREW_PITCH_UM);
      ...
      } else if (c == commands[static_cast<int>(CommandKey::L)]
                              [0])  // 'L' Show position status
      {
        if (!isMoving()) showPositionStatus();
      } else if (c == commands[static_cast<int>(CommandKey::K)]
                              [0])  // 'K' Show encoder interrupt counters
      {
        if (!isMoving()) {
          ...
          if (currentIndex >= 4 || !driverEnabled[currentIndex]) {
            ...
            if (encoderMae3[currentIndex].enable() == mae3::Status::Ok) {
              uint32_t pulses = positionController[currentIndex]
                                    ...
                                    .position_pulse;
              float encoderAngle =
                  UnitConverter::convertFromPulses(pulses).TO_DEGREES;
      ...
      } else if (c == commands[static_cast<int>(CommandKey::J)]
                              [0])  // 'J' Show encoder interrupt counters
      {
        if (!isMoving()) {
          ...
          if (enabled) {
            const uint32_t pulses = positionController[currentIndex]
                                        .getEncoderState()
                                        .position_pulse;
            const float angle =
                UnitConverter::convertFromPulses(pulses).TO_DEGREES;
      ...
      if (c == cmdMotor) {
        ...
        if (!isMoving()) {
          ...
          int32_t currentSteps =
              positionController[currentIndex].getCurrentSteps();
          float position =
              UnitConverter::convertFromSteps(currentSteps).TO_DEGREES;
        ...
        if (!isMoving()) {
          ...
          if (isLinearMotor(currentIndex)) {
            target =
                UnitConverter::convertFromPulses(loaded_pulses).TO_MICROMETERS +
                (loaded_turns * ENCODER_RESOLUTION);
          } else {
        ...
        if (c.getArgument("n").isSet()) {
          ...
          if (encoderMae3[currentIndex].enable() == mae3::Status::Ok) {
            uint32_t pulses = positionController[currentIndex]
                                  ...
                                  .position_pulse;
            ConvertValues encoderPulses =
                UnitConverter::convertFromPulses(pulses);
            Direction direction =
                positionController[currentIndex].getEncoderState().direction;
            MotorStatus motorStatus =
                positionController[currentIndex].getMotorStatus();

            if (motorStatus.currentSteps == 0) {
              positionController[currentIndex].setCurrentPosition(
                  encoderPulses.TO_STEPS);
              motorStatus.currentSteps =
                  positionController[currentIndex].getCurrentSteps();
            }

            ConvertValues currentMotorSteps =
                UnitConverter::convertFromSteps(motorStatus.currentSteps);
            ConvertValues targetMotorSteps =
                UnitConverter::convertFromSteps(motorStatus.targetSteps);
            ...
            if (isLinearMotor(currentIndex)) {
              ...
              float diff = currentMotorSteps.TO_MICROMETERS -
              ...
              log_d(
                  ...
                  "   • Enabled  : %s\n"
                  "   • Mode     : %s\n",
                  currentIndex + 1, currentMotorSteps.TO_MICROMETERS,
                  currentMotorSteps.TO_TURNS, targetMotorSteps.TO_MICROMETERS,
                  targetMotorSteps.TO_TURNS, diff, encoderPulses.TO_MICROMETERS,
                  toString(direction), pulses, movingStr, enabledStr,
                  toString(motorStatus.controlMode));
            } else {
              // For rotational motors, display in degrees
              float diff =
                  currentMotorSteps.TO_DEGREES - encoderPulses.TO_DEGREES;
              origin_turn = 0;
              ...
              log_d(
                  ...
                  "   • Enabled  : %s\n"
                  "   • Mode     : %s\n",
                  currentIndex + 1, currentMotorSteps.TO_DEGREES, 0,
                  targetMotorSteps.TO_DEGREES, 0, diff,
                  encoderPulses.TO_DEGREES, toString(direction), pulses,
                  movingStr, enabledStr, toString(motorStatus.controlMode));
            }
...
void checkDifferenceCorrection() {
  ...
  uint32_t pulses =
      positionController[currentIndex].getEncoderState().position_pulse;
...
void onVoltageDrop() {
  ...
  if (isMoving()) {
    ...
    log_w("Motor %d stopped due to voltage drop, position saved",
          currentIndex + 1);
  }
}
...
void linearProcess(float targetMicrometers)  // amir
{
  ...
  log_d(
      ...
      motorStatus.currentSteps, motor.TO_STEPS, motor.TO_TURNS,
      motor.TO_MICROMETERS, currentSteps,

      targetSteps, target.TO_STEPS, target.TO_TURNS, target.TO_MICROMETERS,

      encoderPulses.TO_DEGREES, encoderPulses.TO_MICROMETERS,

      toString(control_mode),

      voltageDrop_pulses, voltageDrop_turn);

  bool success = positionController[currentIndex].moveToSteps(
      targetSteps, MovementType::MEDIUM_RANGE, control_mode);
...
void rotationalProcess(float targetAngle) {
  ...
  log_d(
      ...
      "   • Mode         : %s\n"
      "   • Target       : %f steps\n",
      currentIndex + 1, encoderPulses.TO_STEPS, encoderPulses.TO_DEGREES,
      voltageDrop_pulses, voltageDrop_turn, toString(control_mode),
      targetSteps);
...
void disable() { positionController[currentIndex].disable(); }
void enable() { positionController[currentIndex].enable(); }
void stop() { positionController[currentIndex].stop(); }
void showPositionStatus() {
  uint32_t pulses =
      positionController[currentIndex].getEncoderState().position_pulse;
  ConvertValues encoderPulses = UnitConverter::convertFromPulses(pulses);
  Direction direction =
      positionController[currentIndex].getEncoderState().direction;
  MotorStatus motorStatus =
      positionController[currentIndex].getMotorStatus();  // amir
  ...
  if (isLinearMotor(currentIndex)) {
    ...
    float diff_encoder =
        currentMotorSteps.TO_MICROMETERS - encoderPulses.TO_MICROMETERS;
    int32_t turn = currentMotorSteps.TO_TURNS;
    ...
    log_d(
        ...
        motorStatus.currentSteps, currentMotorSteps.TO_STEPS,
        currentMotorSteps.TO_TURNS, currentMotorSteps.TO_MICROMETERS,

        motorStatus.targetSteps, targetMotorSteps.TO_STEPS,
        targetMotorSteps.TO_TURNS, targetMotorSteps.TO_MICROMETERS,

        diff_steps,

        encoderPulses.TO_DEGREES, encoderPulses.TO_MICROMETERS, diff_encoder,
        toString(direction),

        movingStr, enabledStr,
    ...
    log_d(
        ...
        "   • Mode     : %s\n",
        currentIndex + 1, currentMotorSteps.TO_DEGREES, 0,
        targetMotorSteps.TO_DEGREES, 0, diff, encoderPulses.TO_DEGREES,
        toString(direction), pulses, movingStr, enabledStr,
        toString(motorStatus.controlMode));
  }
}
...
String isSave(size_t result) { return (result == 0) ? "Failed ❌" : "OK ✅"; }
...
inline ControlMode safeModeFromInt(int v, ControlMode fallback) {
  if (v < static_cast<int>(ControlMode::OPEN_LOOP) ||
      v > static_cast<int>(ControlMode::HYBRID)) {
    return fallback;
  }
  return static_cast<ControlMode>(v);
}

bool isMoving() { return positionController[currentIndex].isMoving(); }
...
static void printConversionTable(float inputValue, const ConvertValues& cvfd,
  ...
  Serial.printf("| %-20s | %10s | %10s | %10s | %10s | %12s |\n", "From (unit)",
                "Degrees", "Pulses", "Steps", "Turns", "Micrometers");
  line();

  // From Degrees (inputValue interpreted as degrees)
  Serial.printf("| %-20s | %10.2f | %10ld | %10ld | %10ld | %12.3f |\n",
                "Degrees", cvfd.TO_DEGREES, static_cast<long>(cvfd.TO_PULSES),
                static_cast<long>(cvfd.TO_STEPS),
                static_cast<long>(cvfd.TO_TURNS),
                static_cast<double>(cvfd.TO_MICROMETERS));

  // From Pulses (inputValue interpreted as pulses)
  Serial.printf(
      "| %-20s | %10.2f | %10ld | %10ld | %10ld | %12.3f |\n", "Pulses",
      static_cast<double>(cvfp.TO_DEGREES), static_cast<long>(cvfp.TO_PULSES),
      static_cast<long>(cvfp.TO_STEPS), static_cast<long>(cvfp.TO_TURNS),
      static_cast<double>(cvfp.TO_MICROMETERS));

  // From Steps (inputValue interpreted as steps)
  Serial.printf(
      "| %-20s | %10.2f | %10ld | %10ld | %10ld | %12.3f |\n", "Steps",
      static_cast<double>(cvfs.TO_DEGREES), static_cast<long>(cvfs.TO_PULSES),
      static_cast<long>(cvfs.TO_STEPS), static_cast<long>(cvfs.TO_TURNS),
      static_cast<double>(cvfs.TO_MICROMETERS));

  // From Micrometers (inputValue interpreted as µm)
  Serial.printf(
      "| %-20s | %10.2f | %10ld | %10ld | %10ld | %12.3f |\n", "Micrometers",
      static_cast<double>(cvfm.TO_DEGREES), static_cast<long>(cvfm.TO_PULSES),
      static_cast<long>(cvfm.TO_STEPS), static_cast<long>(cvfm.TO_TURNS),
      static_cast<double>(cvfm.TO_MICROMETERS));
