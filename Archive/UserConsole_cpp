...
// See: include <ESP32Console.h>, Console console; registerCommand(); begin();
...
bool UserConsole::Begin(const ConsoleConfig& cfg) noexcept {
  ...
  g_console.setPrompt(
      cfg.prompt);  // allowed pre-begin. :contentReference[oaicite:3]{index=3}
  ...
  if (cfg.register_builtin_system_cmds) {
    g_console
        .registerSystemCommands();  // help, reboot, sysinfo, meminfo, etc.
                                    // :contentReference[oaicite:4]{index=4}
  }

  // Register project-specific commands (short, non-blocking)
  g_console.registerCommand(
      ConsoleCommand("status", &UserConsole::CmdStatus,
                     "Show brief CNC status and resources."));
  g_console.registerCommand(
      ConsoleCommand("echo", &UserConsole::CmdEcho,
                     "Echo arguments back (usage: echo <text...>)"));
  g_console.registerCommand(
      ConsoleCommand("verbose", &UserConsole::CmdVerbose,
                     "Set verbosity (usage: verbose <0|1|on|off|true|false>)"));

  started_.store(true);
  // Friendly banner (printf is recommended with ESP32Console)
...
// NEW: Register custom commands without exposing internal console types to the
// outside
bool UserConsole::RegisterCommand(const char* name, int (*handler)(int, char**),
                                  const char* help) noexcept {
  if (!started_.load()) {
    return false;
  }
  g_console.registerCommand(ConsoleCommand(name, handler, help));
  return true;
}
...
bool UserConsole::ParseBooleanToken(const char* token, bool& out) noexcept {
  ...
  const struct KV {
    const char* key;
    bool val;
  } table[] = {{"1", true},      {"0", false},       {"on", true},
               {"off", false},   {"true", true},     {"false", false},
               {"enable", true}, {"disable", false}, {"yes", true},
               {"no", false}};
  for (const auto& kv : table) {
    // strcasecmp is available; if not, implement a simple case-insensitive
    // compare
...
int UserConsole::CmdStatus(int argc, char** /*argv*/) {
  ...
  std::printf("uptime=%lus free_heap=%u bytes verbose=%s\n",
              static_cast<unsigned long>(s), static_cast<unsigned>(free_heap),
